...
| SRS-F11 | The system shall target approximately 4,500–5,500 spoken words and at least 30 minutes of measured audio for every scheduled edition. | Active stories receive product-matched historical CVE context. Quiet days use a rotating, cited historical CVE reference set. Rendering fails below 1,800 seconds; invented or repetitive filler is prohibited. | FR-2, FR-6, §11 |
...
**Resolved quiet-day rule:** If collection succeeds but no eligible current items exist, publish a cited historical database-CVE reference edition with an explicit no-current-updates coverage note. If all current and historical evidence is unavailable or withheld, stop publication and notify the operator.
...
| SRS-F15 | The system shall render cover art and burned-in captions synchronized with actual narration timing. | Video decodes with an audio stream; captions use sentence-sized cues timed from their exact synthesized audio chunks, match approved narration, and are monotonic and bounded by duration. Proposed format: MP4/H.264/AAC, 1920×1080. | FR-7, §15 |
