...
def _generate_episode_once(stories, model, edition, api_key=None, minimum_words=0):
    ...
    payload = {"model": model, "temperature": 0.1, "max_completion_tokens": 2000, "reasoning_effort": "low", "messages": [{"role": "system", "content": system}, {"role": "user", "content": f"Return JSON for this EVIDENCE. The complete spoken text must contain at least {minimum_words} words when minimum_words is nonzero:\n" + json.dumps(packet, ensure_ascii=False)}], "response_format": {"type": "json_schema", "json_schema": schema()}}
