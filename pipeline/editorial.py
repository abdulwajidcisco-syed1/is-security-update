...
def validate_episode(episode, claims, urls, minimum_words=0, maximum_words=0):
    ...
    if maximum_words and episode["word_count"] > maximum_words:
...
def _generate_episode_once(stories, model, edition, api_key=None, minimum_words=0, maximum_words=0):
    if not stories:
...
def generate_episode(stories, model, edition, api_key=None, minimum_words=0, maximum_words=0):
    ...
    chapter_maximum = (maximum_words + chapter_count - 1) // chapter_count if maximum_words else 0
    chapters = []
    for index, batch in enumerate(batches):
        if index:
            time.sleep(61)
        chapters.append(_generate_episode_once(batch, model, edition, api_key, chapter_minimum, chapter_maximum))
    combined = {
    ...
    return validate_episode(combined, claims, urls, minimum_words, maximum_words)
